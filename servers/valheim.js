...
function login (authToken, json) {
  ...
  if (config['config']['valheim']['enabled']) {
...
function queryData () {
  GameDig.query({
    port: config['config']['valheim']['port'],
    socketTimeout: config['config']['valheim']['socketTimeout'],
    attemptTimeout: config['config']['valheim']['attemptTimeout'],
    maxAttempts: config['config']['valheim']['maxAttempts'],
    givenPortOnly: config['config']['valheim']['givenPortOnly'],
    type: config['config']['valheim']['type'],
    host: config['config']['valheim']['host'],
    debug: config['config']['valheim']['debug'],
    requestRules: config['config']['valheim']['request-rules']
  }).then((state) => {
    if (state.ping > config['config']['valheim']['PingCheckThreshold']) {
      data = config['config']['valheim']['degraded'] + state.ping + " ping"
      updateStats(process.env.QUERY_TIMEOUT)
    } else {
      data = config['config']['valheim']['online'] + state.players.length + "\/" + state.maxplayers
      updateStats(process.env.QUERY_TIMEOUT)
    }
  }).catch((exception) => {
    data = config['config']['valheim']['offline']
